...
describe('FileSessionStorage maxStoredMessages', () => {
  ...
  beforeEach(() => {
    tmpDir = `/tmp/agentpack-test-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  });
...
describe('FileSessionStorage list', () => {
  ...
  beforeEach(() => {
    tmpDir = `/tmp/agentpack-test-list-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  });
...
describe('FileSessionStorage key 编码', () => {
  ...
  beforeEach(() => {
    tmpDir = `/tmp/agentpack-test-enc-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  });
...
describe('FileSessionStorage baseDir', () => {
  ...
  it('未指定时使用默认路径', () => {
    ...
    assert.equal(store.dir, path.join(process.cwd(), '.agentpack', 'sessions'));
  });
});
...
describe('FileSessionStorage 原子写入', () => {
  ...
  beforeEach(() => {
    tmpDir = `/tmp/agentpack-test-atomic-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  });
...
describe('FileSessionStorage delete', () => {
  ...
  beforeEach(() => {
    tmpDir = `/tmp/agentpack-test-del-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  });
...
describe('FileSessionStorage 容错', () => {
  ...
  beforeEach(() => {
    tmpDir = `/tmp/agentpack-test-corrupt-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  });
