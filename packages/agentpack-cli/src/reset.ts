/**
 * packages/agentpack-cli/src/reset.ts
 *
 * 重置数据：all / config / logs / sessions / memory（带确认提示）。
 * 对齐 src/cli.ts 的 reset 命令组，路径基于 agentpack 配置解析。
 */
...
export async function resetAll(skipConfirm: boolean): Promise<void> {
  ...
  console.log('🔄 正在重置所有 agentpack 数据...');
  doResetConfig(paths);
  doResetLogs(paths);
  ...
  console.log('\n✅ 所有数据重置完成。');
  console.log('   运行 "agentpack chat" 重新开始。');
}
