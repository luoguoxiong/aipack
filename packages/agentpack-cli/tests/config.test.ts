...
describe('loadConfig', () => {
  ...
  const originalConfigDir = process.env.AGENTPACK_CONFIG_DIR;
  let tmpDir: string;
  ...
  after(async () => {
    ...
    if (originalConfigDir === undefined) {
      delete process.env.AGENTPACK_CONFIG_DIR;
    } else {
      process.env.AGENTPACK_CONFIG_DIR = originalConfigDir;
    }
  ...
  async function loadIn(dir: string, cli: Parameters<typeof loadConfig>[0] = {}) {
    ...
    // 全局配置目录也隔离在临时目录下
    const globalDir = path.join(absDir, '.agentpack-global');
    await fs.promises.mkdir(globalDir, { recursive: true });
    process.env.AGENTPACK_CONFIG_DIR = globalDir;
  ...
  it('全局配置 config.json 生效', async () => {
    ...
    const globalFile = path.join(cfg.workspace, '.agentpack-global', 'config.json');
  ...
  it('项目级 .json 覆盖全局配置（项目优先）', async () => {
    ...
    await fs.promises.writeFile(
      path.join(process.cwd(), 'agentpack.config.json'),
      JSON.stringify({ provider: 'deepseek', model: 'deepseek-chat' }),
    );
    const loaded = await loadConfig({});
  ...
  it('项目级 .js 优先于 .json', async () => {
    ...
    await fs.promises.writeFile(
      path.join(process.cwd(), 'agentpack.config.js'),
  ...
  it('环境变量覆盖配置文件', async () => {
    ...
    process.env.AGENTPACK_PROVIDER = 'anthropic';
    process.env.AGENTPACK_MODEL = 'claude-sonnet';
    const loaded = await loadConfig({});
    assert.equal(loaded.provider, 'anthropic');
    assert.equal(loaded.model, 'claude-sonnet');
    delete process.env.AGENTPACK_PROVIDER;
    delete process.env.AGENTPACK_MODEL;
  });
