...
import { createAipackRuntime, resolveModelForCli } from './runtime';
...
program
  .name('aipack')
  .description('aipack - 基于 aipack 框架的 AI 命令行助手')
  .version('0.1.0');

program
  .option('-c, --config <path>', '配置文件路径（支持 .js/.json，默认合并 <cwd>/aipack.config.js 与 ~/.aipack/config.json）')
...
async function init(): Promise<InitResult> {
  ...
  if (!hasAnyApiKey()) {
    if (!process.stdin.isTTY) {
      ...
      console.error('   请在交互式终端运行 "aipack chat" 进行首次配置，');
      console.error('   或在环境变量中配置，例如：export DEEPSEEK_API_KEY="your-key"');
      console.error('   或写入 ~/.aipack/.env / <cwd>/.env 文件');
      console.error('   运行 "aipack models" 查看支持的提供商与模型');
      process.exit(1);
    }

    // 向导内部会写入 API Key + AIPACK_PROVIDER/MODEL 到环境变量与 .env
...
function printInitInfo(config: AipackConfig, model: AiModel): void {
  console.log('✅ aipack 初始化成功');
...
program
  ...
  .action(async (sessionKey: string) => {
    ...
    if (!stored) {
      ...
      console.error('   运行 "aipack sessions list" 查看已有会话');
      process.exit(1);
    }
...
program
  .command('init')
  .description('初始化配置文件（交互式生成 aipack.config.js / config.json）')
  .option('-g, --global', '写入全局配置 ~/.aipack/config.json')
  .option('-l, --local', '写入项目级配置 <cwd>/aipack.config.js')
...
const resetCmd = program.command('reset').description('重置 aipack 数据（配置、日志、会话、记忆）');
