 ...
 *
 * 模型解析 + Runtime 工厂。
 * 基于 aipack 框架封装：getBuiltinModel + adaptAiModel / createStreamFnFromAi
 ...
 * 4. 全部失败则抛出可读错误
 */
export function resolveAiModel(config: AipackConfig): AiModel {
 ...
 * - 完全无 Key 时打印可读错误并退出
 */
export function resolveModelForCli(config: AipackConfig): AiModel {
...
/**
 * 创建 aipack Runtime（可选传入已解析的模型，避免二次解析不一致）
