 ...
 *
 * 初始化配置文件向导：交互式生成项目级 aipack.config.js 或全局 ~/.aipack/config.json。
...
function getProjectConfigPath(): string {
  return path.join(process.cwd(), 'aipack.config.js');
}
...
function buildJsConfigContent(config: RawFileConfig): string {
  ...
  lines.push('');
  lines.push('  // 高级：透传给 aipack Runtime 的选项（.js 配置可 import 模块/类实例）');
  lines.push('  // tools: [],');
...
export async function runInitConfig(
  ...
  if (!process.stdin.isTTY) {
    ...
    console.error('   请直接在终端执行 "aipack init"，');
    console.error('   或手动创建 aipack.config.json / ~/.aipack/config.json 配置文件。');
    process.exit(1);
  }
  ...
  try {
    console.log('');
    console.log('╔══════════════════════════════════════════════╗');
    console.log('║   ⚙️  aipack 初始化配置文件向导           ║');
    console.log('╚══════════════════════════════════════════════╝');
    console.log('');
    ...
    console.log('   运行 "aipack chat" 使用该配置。');
