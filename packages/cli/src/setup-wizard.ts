 ...
 *   1. 选择模型提供商（来自 aipack 内置目录）
 *   2. 输入 API Key
 *   3. 选择模型
 *   4. 保存到 ~/.aipack/.env 并写入 process.env
 *
 * 参考 src/setup-wizard.ts（kobot）的交互风格，适配 aipack 内置提供商/模型。
 */
...
export async function runSetupWizard(): Promise<SetupResult> {
  ...
  try {
    console.log('');
    console.log('╔══════════════════════════════════════════════╗');
    console.log('║   🔑 aipack 首次设置 - 模型配置向导      ║');
    console.log('╚══════════════════════════════════════════════╝');
    console.log('');
    ...
    process.env.AIPACK_PROVIDER = provider.id;
    process.env.AIPACK_MODEL = model.id;

    // 持久化到用户级 .env，供以后启动使用
    const envPath = saveEnvFile({
      [provider.envVar]: apiKey,
      AIPACK_PROVIDER: provider.id,
      AIPACK_MODEL: model.id,
    });
