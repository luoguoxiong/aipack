/**
 * VSCode 配置读取：把 contributes.configuration 中的 aipack.* 设置聚合成 AgentConfig。
 *
 * 调用 syncApiKeysToEnv 把 apiKey 同步到 process.env（复用 aipack 的 getEnvApiKey 兜底），
...
export interface AgentConfig {
  ...
  model: string;
  /** provider id → apiKey（来自 settings aipack.apiKey.*） */
...
export function readAgentConfig(): AgentConfig {
  const cfg = vscode.workspace.getConfiguration('aipack');
