...
export class CodingChatView implements vscode.WebviewViewProvider {
  public static readonly viewType = 'aipack.coding.chatView';
  ...
  resolveWebviewView(
    ...
    console.log('[aipack] resolveWebviewView');
    this.view = view;
  ...
  private async handleMessage(msg: WebviewInbound): Promise<void> {
    console.log('[aipack] msg:', msg.type);
    switch (msg.type) {
      case 'ready':
  ...
  private async runStream(text: string): Promise<void> {
    ...
    console.log('[aipack] runStream start:', text.slice(0, 30));
    this.post({ type: 'userMessage', text });
    this.setRunning(true);

    try {
      console.log('[aipack] stream loop starting');
      for await (const chunk of this.agent.streamRun(text, this.sessionKey)) {
        console.log('[aipack] chunk:', chunk.type);
        this.forwardChunk(chunk);
      }
      console.log('[aipack] stream loop done');
    } catch (err) {
      console.error('[aipack] stream error:', err);
