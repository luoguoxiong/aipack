...
export function registerCommands(
  ...
  ctx.subscriptions.push(
    vscode.commands.registerCommand('agentpack.coding.openChat', async () => {
      if (!vscode.workspace.workspaceFolders?.length) {
        vscode.window.showWarningMessage(
          '请先打开一个文件夹再使用 Agentpack Coding。',
        );
        return;
      }
      chatView.show();
    ...
    vscode.commands.registerCommand('agentpack.coding.stop', async () => {
      await chatView.stop();
    }),

    vscode.commands.registerCommand('agentpack.coding.clearHistory', async () => {
