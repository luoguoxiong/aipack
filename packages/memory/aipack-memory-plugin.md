...
1. **`buildContext` 过滤 `role==='system'` 消息** —— `packages/agent/runtime/index.ts:585` `messages.filter(m => m.role !== 'system')`。故注入必须用 `role:'user'`，`role:'system'` 资源不会到达模型。
...
- `done.tapPromise`：aipack `RuntimeHooks.done` 签名已扩展，框架将最终 `Request` 作为第二参数传入；扩展用 `request.sessionKey` 从 Map 精确取回 stash 并与本轮结果配对 —— 彻底解决旧 FIFO 设计在并发多会话下错配/丢失的问题。框架保证同一 sessionKey 的 run 串行执行，故 Map 键控安全。
...
4 个 Tool（`Tool`/`createTextContent` 来自 `aipack`，`parameters` 用**纯 JSON Schema 对象**，不依赖 TypeBox）：`save_memory(content, concepts?)` / `search_memory(query, limit?)` / `list_memories(limit?)` / `delete_memory(id)`。`execute` 返回 `ToolResult`。
...
默认 store=`FileMemoryStore`、retriever=`HybridRetriever`（无 embedder 纯 BM25）、capture=true、inject=true、tools=true。`install()` 返回三数组，供 `aipack.config.js` 展开到 `extensions`/`transformers`/`tools`。插件层将 `StoreBackedRetriever`（包装 store.search）与 `StoreVectorSource`（包装 store.searchVectors）装配进 HybridRetriever，注入 transformer 的 `onRecall` 装配为 `store.touchRecall`（fire-and-forget）。

### `index.ts`

聚合 export 全部类型与实现；从 `aipack` 再导出 `Extension`/`ContextTransformer`/`Tool`/`ToolResult` 类型方便单一 import。
...
- `package.json`：`"type":"module"`、tsup、`exports` map、`engines.node>=18`、`peerDependencies: { aipack: "workspace:*" }`、`devDependencies: { aipack, @types/node, tsup, typescript }`。发布到 npm 时 `workspace:*` → `"^0.1.0"`。**零其它运行时依赖**。
- `tsup.config.ts` 镜像 aipack：`format:['esm']`、`target:'es2022'`、`dts:true`、`skipNodeModulesBundle:true`。
- aipack 全程 `import type`，编译后类型擦除，运行时不打进本包，仅 peer 解析。

## 实现顺序

1. `package.json`/`tsconfig.json`/`tsup.config.ts`（镜像 aipack）
2. `src/types.ts`
...
9. 运行：`node --import tsx packages/memory/examples/round-trip.ts`。
