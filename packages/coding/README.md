...
aipack 的 **coding 工具集 + coding agent 工厂 + CLI**。
...
- `aipack-coding` CLI — 交互式 coding REPL + 一次性 run
...
```bash
npm install aipack-coding
# 或
pnpm add aipack-coding
```
...
```ts
import { createRequest } from 'aipack';
...
# 一次性执行
aipack-coding run "给 src/utils.ts 加上 JSDoc 注释"

# 指定模型与工作区
aipack-coding -p openai -m gpt-4o-mini -w /path/to/repo

# 启用记忆集成
aipack-coding --memory chat
aipack-coding --memory run "总结项目结构"
```

`--memory` 标志会动态加载 `aipack-memory` 插件，为会话注入持久化记忆能力（需已安装 `aipack-memory`）。
...
```ts
import { createCodingAgent, PermissionManager } from 'aipack-coding';
...
const agent = await createCodingAgent({
  provider: 'deepseek',
  model: 'deepseek-chat',
  memory: { baseDir: '~/.aipack/memory' },  // 动态 import aipack-memory
});
```
...
```bash
pnpm install
pnpm --filter aipack-coding typecheck   # 类型检查
pnpm --filter aipack-coding build       # 构建（产出 dist/index.js + dist/cli.js）
pnpm --filter aipack-coding example     # 零依赖往返验证
```

## 相关项目

- [aipack](../aipack/README.md) — Agent 框架
- [aipack-cli](../aipack-cli/README.md) — 通用 CLI
- [aipack-memory](../aipack-memory/README.md) — 持久化记忆插件
