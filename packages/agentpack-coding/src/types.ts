/**
 * agentpack-coding 类型定义。
 *
 * CodingToolContext 贯穿所有工具，提供 workspace 根目录与权限管理器；
 * 各工厂函数的 options 与返回类型也在此声明。
 */

import type { Tool, Extension, ContextTransformer } from 'agentpack';
...
export interface CodingPlugin {
  ...
  /** 返回 tools + transformers，供 agentpack.config.js 展开 */
...
export interface CodingAgentOptions {
  ...
  /** 启用的工具子集（默认全部 7 个） */
  enabledTools?: string[];
  /** 启用 agentpack-memory 集成（动态 import，避免硬依赖） */
...
export interface CodingAgent {
  /** 装配好的 runtime */
  runtime: import('agentpack').Runtime;
  /** 权限管理器（编程式调用） */
  permission: PermissionManager;
  /** 工具列表（已注册到 runtime） */
  tools: Tool[];
