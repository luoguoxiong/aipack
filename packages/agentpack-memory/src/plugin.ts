 ...
 * 提供 embedder 后自动升级为 BM25 + 向量混合检索；提供 summarizeFn 后 capture 走 LLM 摘要。
 */

import type { Extension, ContextTransformer, Tool } from 'agentpack';
import { FileMemoryStore } from './store/file-memory-store';
import { HybridRetriever } from './retrieval/hybrid-retriever';
import type { RetrieverLike } from './retrieval/hybrid-retriever';
...
export interface MemoryPluginOptions {
  /** FileMemoryStore 存储目录（支持 ~ 开头），默认 <cwd>/.agentpack/memory */
  baseDir?: string;
  ...
  embedder?: Embedder;
  /** 可选 LLM 摘要函数，配置后 capture 走 LLM 摘要（默认零-LLM 抽取） */
  summarizeFn?: SummarizeFn;
  /** 每 N 次捕获自动触发一次 consolidate（0=不自动，默认 0） */
  consolidateOn?: number;
}
...
export interface MemoryPlugin {
  ...
  tools: Tool[];
  /** 返回 { extensions, transformers, tools }，供 agentpack.config.js 展开 */
  install(): {
    extensions: Extension[];
    transformers: ContextTransformer[];
    tools: Tool[];
...
function resolveCaptureOptions(
  ...
  fallback: { summarizeFn?: SummarizeFn; consolidateOn?: number },
): CaptureOptions {
  if (typeof v === 'object' && v !== null) {
    return {
      ...v,
      summarizeFn: v.summarizeFn ?? fallback.summarizeFn,
      consolidateEvery: v.consolidateEvery ?? fallback.consolidateOn,
    };
  }
  return {
    summarizeFn: fallback.summarizeFn,
    consolidateEvery: fallback.consolidateOn,
  };
}
...
export function createMemoryPlugin(options: MemoryPluginOptions = {}): MemoryPlugin {
  ...
  // 2. Retriever：用 StoreBackedRetriever 包装 store，保证数据实时同步
  ...
  // 3. Consolidator：注入 store，使 store.consolidate() 可用
  ...
  if (isCaptureEnabled(options.capture)) {
    const captureOpts = resolveCaptureOptions(options.capture, {
      ...
      consolidateOn: options.consolidateOn,
    });
  ...
  if (options.tools !== false) {
    tools.push(...createMemoryTools(store));
  }
