...
const LazyDiffSyntaxHighlight = lazy(async () => {
  ...
  return {
    default: function LoadedDiffSyntaxHighlight({
      ...
      const SyntaxHighlighterComponent = SyntaxHighlighter as unknown as React.ComponentType<{
        language?: string;
        ...
        "data-testid"?: string;
        renderer?: (args: { rows: unknown[]; stylesheet: unknown; useInlineStyles: boolean }) => React.ReactNode;
          ...
          renderer={({ rows, stylesheet, useInlineStyles }) => (
            <DiffLineTable
              ...
              renderCode={(line, index) => {
                const node = (rows as SyntaxNode[])[index];
                if (!node) return line.content || " ";
                return createSyntaxElement({
                  node: trimTrailingLineBreak(node),
                  stylesheet: stylesheet as Record<string, React.CSSProperties>,
                  useInlineStyles,
                  key: `diff-code-${index}`,
                }) as React.ReactNode;
              }}
