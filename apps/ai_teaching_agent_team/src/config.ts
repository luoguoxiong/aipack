...
export function loadConfig(): AppConfig {
  ...
  if (!llmReady) {
    ...
    console.warn('   设置后重试,例如: DEEPSEEK_API_KEY=sk-xxx pnpm --filter ai-teaching-agent-team dev');
    console.warn('   无 Key 时仍可启动服务,但 /api/teach 会返回错误提示。\n');
  }
  ...
  return {
    ...
    serpapiKey,
    llmReady,
    searchBackend: describeSearchBackend(serpapiKey),
    models,
  };
}
