/**
 * apps/ai_office_agent/src/config.ts
 *
 * 环境变量解析 + aipack 模型/streamFn 装配 + 文件工作区路径解析。
 * 对齐 ai_travel_agent/config.ts 的「getBuiltinModel → adaptAiModel → createStreamFnFromAi」模式。
 * 多层容错:缺 Key 时给出明确提示而非崩溃,服务仍可启动。
 */
...
export function loadConfig(): AppConfig {
  ...
  // 文件工作区:相对本 app 目录解析(OFFICE_WORKSPACE 可覆盖)
  const workspace = path.resolve(
    __dirname,
    '..',
    process.env.OFFICE_WORKSPACE || 'office-workspace',
  );
  ...
  if (!llmReady) {
    ...
    console.warn('   设置后重试,例如: DEEPSEEK_API_KEY=sk-xxx pnpm --filter ai-office-agent dev');
    console.warn('   无 Key 时仍可启动服务,但 /api/chat 会返回错误提示。\n');
  }
